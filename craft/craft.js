		...
		let getGeometry = function(event,pointer){
			...
			if(!zoneInstance){ //as icon
				if(event){
					...
					let diffX=geometry.left-(geometry.width/2) //-(Math.abs(geometry.width-minWidth)/2)
					let diffY=geometry.top-(geometry.height/2) //-(Math.abs(geometry.height-minHeight)/2)
