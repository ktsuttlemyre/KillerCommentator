window.SourceManager=(function(document,SourceManager,pp){let inject=pp.inject, appendTo=pp.appendTo, prependTo=pp.prependTo, ajax=pp.ajax, domParse=pp.domParse;
	let config = JSON.parse(localStorage.getItem('KillerCommentator.config') || showOpenFilePicker() || `{
		"urls":{
			"challonge":[]
		},
		"twitch":{
...
// 				}, false )
	...
	appendTo(document.body,inject('script',{src:"https://unpkg.com/@ungap/custom-elements-builtin"},function(){
		appendTo(document.body,inject('script',{src:"https://unpkg.com/x-frame-bypass", type:"module"},function(){
			...
			ajax(KillerCommentator.base_site+"carousel_partners.html",function(html){
				...
				const carousel = new bootstrap.Carousel('#Carousel_partners')
			});
	...
	appendTo(document.body,inject('script',{src:"https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/js/bootstrap.min.js", integrity:"sha384-w76AqPfDkMBDXo30jS1Sgez6pr3x5MlQ1ZAGC+nuZB+EYdgRZgiwxhTBTkF7CXvN", crossorigin:"anonymous"},function(){
